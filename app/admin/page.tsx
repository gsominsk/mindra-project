        ...
        <div className={`min-h-screen transition-colors duration-500 ${theme.bg} ${state.eventType === 'party' ? 'text-white' : 'text-gray-900'}`}>
            ...
            <header className={`sticky top-0 z-50 backdrop-blur-md border-b ${state.eventType === 'party' ? 'border-gray-800 bg-slate-900/80' : 'border-gray-200 bg-white/80'} px-6 py-4 flex justify-between items-center`}>
                <div className="flex items-center gap-2">
                    ...
                    <span className={`font-bold text-lg hidden sm:block ${theme.font}`}>PageBuilder</span>
                </div>
                <div className="flex gap-3">
                    <button className={`hidden sm:block px-4 py-2 rounded-lg text-sm font-medium transition-colors ${state.eventType === 'party' ? 'bg-gray-800 hover:bg-gray-700 text-gray-200' : 'bg-white border hover:bg-gray-50 text-gray-700'}`}>
                        Preview
                    </button>
                    <button className={`px-4 md:px-6 py-2 rounded-lg text-sm font-medium text-white shadow-lg shadow-blue-500/20 transition-all transform hover:-translate-y-0.5 ${theme.button}`}>
                        Publish
                    </button>
                </div>
            </header>
