...
## 12. Разработка UI для модерации (Admin Sync UI)

После стабилизации Backend ETL пайплайна, был успешно внедрен механизм модерации неструктурированных сырых Instagram-постов (Drafts / Uncategorized).

### Решенные задачи интеграции:
1. **Сохранение Инста-дат (Chronological Fidelity):**
   - Python-маппер и `queue/route.ts` доработаны для сквозного сохранения `post.timestamp` через Prisma в `EventPage.createdAt`. 
   - Это гарантирует, что дата отображаемого события на сайте соответствует оригинальной публикации в Instagram.
2. **Drafts ("Uncategorized") по умолчанию:**
   - Новый `isPublished = False` дефолтный флаг во всех моделях. Ни одна LLM генерация больше не попадет на public сайт без аппрува модератора.
3. **Queue / Drafts Вкладка:**
   - В UI выведено отдельное представление для статуса `uncategorized` с превью текста (первые 80 символов) для модерации. Снапшот текста позволает понять тип события без входа в `PageEditor`.
4. **Быстрые Действия (Quick Publish Actions):**
   - Реализован метод `PATCH /api/admin/pages/[id]`. Прямо из карточки дашборда пост переназначается нужной категории (`Business` / `Wedding` / `Party`) и моментально публикуется.

Весь тест-сьют бэкенда (`synс-tests`) полностью адаптирован и проходит со $100\%$ покрытием. Мост между Instagram ETL и CMS Frontend Editor официально признан стабильным.
